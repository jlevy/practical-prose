...
- The applicable dimensions depend on `risk_level`. Low-stakes notes audit only P1 / E1
  ...
  See [practical-prose-metrics.md](../docs/practical-prose-metrics.md) §Applicability
  Profiles.
...
- [ ] **R1 Discipline:** Observation, judgment, interpretation, implication worked
  through in order, each higher rung supported by the prior?
  Each rung carries its own evidence?
...
- [ ] **R4 Parsimony:** Each load-bearing reasoning chain the minimum sufficient sound
  argument? No non-load-bearing rungs; no weaker warrant where a stronger one was
  available; re-derivations only where they add inspectability, confidence, or audience
...
- [ ] **G3 Relevance:** Every cited source and every section bears on the document’s
  purpose? Digressions marked as digressions?
  Each source passes the one-sentence test ("this source supports claim X, which bears
  on purpose Y")?
...
Where the discipline applies, do not combine passes.
