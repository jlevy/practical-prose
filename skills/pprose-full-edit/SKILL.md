...
---
<!-- DO NOT EDIT: generated by `pprose install` (format=f01 surface=skill-md) -->

> Run pprose as `pprose <command>` if on PATH, else `uvx pprose@0.1.0 <command>` (zero-install via uv). Run `pprose --help` for every command, `pprose skill --list` for the other Practical Prose skills, and `pprose shortcut --list` / `pprose guidelines --list` / `pprose runbook --list` for on-demand playbooks, style guides, and procedures.
