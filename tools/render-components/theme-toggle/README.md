...
A pill-shaped three-button toggle that controls `data-theme` and `data-theme-mode` on
`<html>`. Auto tracks `prefers-color-scheme`; Light and Dark are explicit overrides.
...
[theme-toggle.html.jinja](theme-toggle.html.jinja) provides the three-button markup.
Include it via `{% include 'theme-toggle.html.jinja' %}` in any Jinja template that
needs the toggle.

## Print behavior

The component carries no print rules.
Surfaces that need to hide the toggle in printed output should include something like:
...
The `mountSurfaceToggle` half stays in the workbench’s local `lib/` since the
