...
The two side panels that mirror the rubric (Detail) and the per-doc
evaluation (Assessment) for whatever element the cursor is over. The card
component emits `[data-tip-kind="dim|group|score"]` triggers and these
panels listen on the document; hovering a dim updates both panels, hovering
a group updates just the Detail panel.
...
- `window.marked` — markdown library used for content rendering. Vendored
  at [../vendor/marked.min.js](../vendor/marked.min.js).
...
The component appends a `.tip-panel-heading` and a `.tip-content` div to
each; CSS keys off `.is-empty` to hide chrome when there's nothing to show.
