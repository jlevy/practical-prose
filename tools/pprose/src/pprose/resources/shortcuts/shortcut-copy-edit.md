...
2. Read the Expression and Form dimensions of `practical-prose-guidelines.md`
   (§Expression Dimensions, E1-E3, and §Form Dimensions, F1-F3): E1 Clarity
   (banned-register words, vague magnitudes, meta-commentary, parallel-structure
   padding), E2 Coherence, E3 Concision, F1 Organization, F2 Consistency, F3 Formatting.

3. Read `ai-prose-corrections.md` — the binding catalog of LLM-register tells (lexical
   tells, structural patterns, attention flags) with the correction for each.
