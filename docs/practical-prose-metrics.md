...
| 14 | Inference Discipline | Rung-tag count (`[observed]`, `[judged]`, `[interpreted]`, `[implied]`) in audit/eval modes; multi-rung-per-sentence flag | Are observation, judgment, interpretation, implication kept distinct? | metrics.py bracket tags (audit mode); LLM-assist; manual |
...
| 17 | Parsimony | Count of chains where a shorter sound chain exists (citable fact re-derived, weaker warrant where a stronger one is available); count of non-load-bearing rungs flagged within load-bearing chains; per-doc parsimony-gap flag count | For each load-bearing chain, is it the minimum sufficient given its purpose and per-step warrants? | LLM-assist; manual |
...
| --- | --- | --- | --- | --- |
| **Low-stakes note** (status update, standup, brief progress note) | `low` | §1 Suitability, §5 Clarity, §7 Concision, §10 Formatting | §11 Verifiability only for material claims | §13 Relevance, §14-§17 (Inference Discipline, Soundness, Precision, Parsimony), §18-§20 (Calibration, Fairness, Robustness) |
| **Standard internal doc** (memo, brief, internal report) | `standard` | All Purpose (§1-§4); all Expression (§5-§10); §11 Verifiability; §12 Factuality; §15 Soundness | §13 Relevance, §14 Inference Discipline, §17 Parsimony, §18 Calibration, §19 Fairness, §20 Robustness when the doc makes those kinds of claims | §16 Precision unless terminology is contested |
