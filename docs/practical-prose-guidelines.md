...
7. **Em-dash discipline.** *(Common agent failure mode.)* Em dashes are allowed but
   should be rare. Within routine, full sentences, do not use them.
   ...
   When you do use one within a sentence, write it American style, unspaced
   (`context—like this`), never with surrounding spaces.
   Spaced em-dash overuse is a hallmark of LLM-written text.
