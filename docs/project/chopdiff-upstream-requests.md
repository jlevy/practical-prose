...
description: The chopdiff API gaps pprose still works around after the 0.3.1 upgrade, written up so they can be filed as issues against github.com/jlevy/chopdiff before its next release.
date: 2026-06-02
...
File them against [jlevy/chopdiff](https://github.com/jlevy/chopdiff) before the next
chopdiff release; if accepted, pprose drops the corresponding workaround.
...
These are tracked under the structural-metrics epic (`pp-3hg4`) with the
`upstream-chopdiff` label.
